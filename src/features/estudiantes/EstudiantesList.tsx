...
import { schoolQueryKeys, useCursosBySede, useEstudiantesBySede } from '../../hooks/useSchoolQueries';
import { Estudiante } from '../../types';
import {
  Plus, Search, Edit2, Trash2, FolderOpen, FileText
} from 'lucide-react';
import DocumentManager from './DocumentManager';
import EstudianteForm from './EstudianteForm';

const MatriculaPreviewModal = lazy(() => import('../matricula/MatriculaPreviewModal'));
...
export default function EstudiantesList() {
  ...
  const { data: cursos = [], isLoading: isLoadingCursos } = useCursosBySede(sede.id);
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: schoolQueryKeys.estudiantes(sede.id) });
  ...
  const deactivateMutation = useMutation({
    mutationFn: (id: number) => EstudianteService.deactivate(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: schoolQueryKeys.estudiantes(sede.id) });
      addToast('Estudiante desactivado exitosamente', 'success');
    },
    onError: (err: any) => {
      addToast('Error al desactivar estudiante: ' + err.message, 'danger');
    }
  });

  const handleEdit = (est: Estudiante) => {
    if (!can(PERMISSIONS.ENROLL_STUDENTS)) return;
    setEditingId(est.id);
    setShowForm(true);
  };

  const handleDeactivate = (est: Estudiante) => {
    if (!can(PERMISSIONS.ENROLL_STUDENTS)) return;
    const confirmed = confirm(`¿Desactivar a ${est.nombre} ${est.apellidoPaterno}?`);
    if (confirmed) deactivateMutation.mutate(est.id);
  };

  const { data: estudiantes = [], isLoading } = useEstudiantesBySede(sede.id);
  ...
  const filtered = useMemo(() => {
    return estudiantes.filter(e => {
      ...
      const matchCurso = !filterCurso || e.cursoId === filterCurso;
      return matchSearch && matchCurso;
    }).sort((a, b) => a.apellidoPaterno.localeCompare(b.apellidoPaterno));
  }, [estudiantes, search, filterCurso]);

  if (isLoading || isLoadingCursos) return <div className="page-container"><div className="skeleton" style={{ height: 400 }} /></div>;
                ...
                <div style={{ display: 'flex', gap: '4px' }}>
                  {can(PERMISSIONS.ENROLL_STUDENTS) && (
                    <>
                      ...
                      <button className="btn btn-ghost" style={{ padding: '8px', color: 'var(--danger)' }} onClick={() => handleDeactivate(est)} disabled={deactivateMutation.isPending}><Trash2 size={18} /></button>
                    </>
                  )}
                </div>
              </div>
