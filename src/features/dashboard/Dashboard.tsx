...
import { useDashboardStats } from '../../hooks/useSchoolQueries';
import { SCHOOL } from '../../config/sedes';
import {
...
export default function Dashboard() {
  const { sede } = useSede();
  const { data: stats, isLoading, error, refetch } = useDashboardStats(sede.id);
    ...
    return (
      <div className="page-container">
        <div className="card" style={{ padding: '40px', textAlign: 'center' }}>
          ...
          <p className="text-muted">No se pudieron cargar las estadisticas.</p>
          <button className="btn btn-navy" onClick={() => refetch()} style={{ marginTop: '20px' }}>
            Reintentar
          </button>
        </div>
      </div>
    ...
    <div className="page-container animate-in">
      <header className="page-header" style={{ marginBottom: '32px' }}>
        <div>
          ...
          <p className="page-subtitle">Principales metricas de gestion para {sede.nombre}</p>
        </div>
      </header>
      ...
      <section className="grid grid-3" style={{ marginBottom: '32px' }}>
        ...
        <div className="card" style={{ padding: '24px', display: 'flex', alignItems: 'center', gap: '20px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '28px', fontWeight: 900, color: 'var(--color-navy)' }}>{Math.round((stats.totalEstudiantes / SCHOOL.capacidadTotal) * 100)}%</div>
          </div>
        </div>
      </section>
      ...
      <div className="grid grid-2">
        <div className="card" style={{ padding: '32px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
            <h3 style={{ fontSize: '18px', fontWeight: 800 }}>Distribucion por Nivel</h3>
            <div className="badge badge-navy">Libro Matricula</div>
          </div>
          <ResponsiveContainer width="100%" height={300} minWidth={0}>
            <BarChart data={chartData}>
              ...
              <Tooltip cursor={{fill: 'rgba(0,0,0,0.02)'}} contentStyle={{borderRadius: '16px', border: 'none', boxShadow: 'var(--shadow-lg)', padding: '12px'}} />
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            ...
            <button className="btn btn-outline" style={{ justifyContent: 'space-between', padding: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div style={{ textAlign: 'left' }}>
                  <div style={{ fontWeight: 700 }}>Exportar Libro Matricula</div>
